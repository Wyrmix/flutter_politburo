...
class CardSettingsPhotoPicker extends StatefulWidget {
  final List<String> startingUrls;
  final ImageUploader uploader;

  CardSettingsPhotoPicker(this.uploader, {this.startingUrls = const []});

  @override
  _CardSettingsPhotoPickerState createState() => _CardSettingsPhotoPickerState();
}

class _CardSettingsPhotoPickerState extends State<CardSettingsPhotoPicker> {
  @override
  Widget build(BuildContext context) {
    Fimber.d("Building image picker card");

    var imagePickers = mutableListOf<ImagePicker>();
    widget.startingUrls.forEach((url) {
      BlocProvider(bloc: ImagePickerBloc(
          widget.uploader, initial: ImagePickerState.url(url)),);
      imagePickers.add(ImagePicker(url: url,));
    });

    Fimber.d("Pickers [$imagePickers]");
    final rows = imagePickers
        .chunked(2)
        .map((pickers) => Row(children: pickers.list,))
        .list;

    Fimber.d("Rows [$rows]");
    return GridView.extent(maxCrossAxisExtent: 172,
      children: imagePickers.list,
      shrinkWrap: true,
